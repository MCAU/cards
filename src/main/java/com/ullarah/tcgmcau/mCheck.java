...
class mCheck {
    ...
    public static Boolean checkSkill( SkillType skill, Integer level, Integer ran ) {

        Integer val = mConfig.getChanceConfig().getInt("chance.skills." + skill.toString());

        if( level > 50 ) val = val - 5;
        else if( level > 100 ) val = val - 10;
        else if( level > 200 ) val = val - 20;
